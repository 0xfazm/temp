# Monthly Merchant Analytics Report

**Merchant:** Petal & Bean  (ID: 41999)  
**Report Date:** 2025-11-30  
**Report Period:** 2025-11  
**Loyalty Type:** Free Loyalty - points

**Merchant Since:** April 09, 2025


---

## Executive Summary



- **Total Customers (universe)**: 292
- **Active (≤60 days)**: 88 (30.1%)
- **At Risk (61–90 days)**: 33 (11.3%)
- **Churned (>90 days)**: 171 (58.6%)
- **Churn rate for 2025-11**: 27.17%



---

## Monthly Loyalty Value

- **Gross Loyalty Value**: 594.265 BD  
  *Total value earned by customers (points, cash, vouchers)*

- **Redemption Costs**: 65.95 BD  
  *Value of rewards claimed (points redeemed, cash redeemed)*

- **Net Loyalty Value**: 528.315 BD  
  *Gross value minus redemption costs*

### 6-Month Forecast (Illustrative)

- **Projected Gross Value**: 1782.82 BD
- **Projected Net Value**: 1610.529 BD

**Method:** survival_adjusted using 3-month historical average  
**Churn-adjusted:** Yes (survival probability applied)
**Discounting:** 10.0% annual

### How to Interpret These Metrics

✅ **Net Loyalty Value** shows program engagement after accounting for reward costs.  
❌ **NOT** profit — excludes product costs, operations, overhead.  
📊 **Use for:** Budgeting loyalty spend, comparing merchant performance.  
⚠️ **Limitations:** 

- Does not capture non-transactional value (brand loyalty, referrals)
- Forecast is directional only, not a financial guarantee



**Trend chart note:** Last 12 calendar months ending at the selected month; missing months are plotted as **0**.

![CLTV Trend](/merchant_41999_cltv.png)

---

## Customer Segmentation

### Status distribution (must reconcile)
**Universe mode:** `qualifying_events_only`  
**Active + At Risk + Churned:** 88 + 33 + 171 = 292  
**Reconciliation:** PASS

![Customer Status](/merchant_41999_status.png)

---

## Top Customers (by 6m forecast, if available)

| Rank | User ID | Status | Days Since Activity | CLTV (month) | Forecast (6m) |
|---:|---:|---|---:|---:|---:|
| 1 | LV5UT4X | Active | 1 | 3.8 | 77.196 |
| 2 | L0LX4MO | Active | 2 | 31.5 | 75.038 |
| 3 | LTGMVTK | Active | 0 | 31.7 | 64.169 |
| 4 | LSMG0OF | Active | 5 | 29.25 | 58.511 |
| 5 | LCLEMWX | Active | 17 | 5.8 | 56.911 |
| 6 | LL3HD26 | Active | 0 | 36.95 | 54.231 |
| 7 | L2UXZE7 | Active | 1 | 11.9 | 50.844 |
| 8 | LDWPHNO | Active | 8 | 9.6 | 50.397 |
| 9 | LWAWFXI | Active | 12 | 7.2 | 45.559 |
| 10 | L4VAPE9 | Active | 21 | 11.4 | 42.73 |


### At-Risk Customers (priority list)

| User ID | Days Since Activity | CLTV (month) | Forecast (6m) |
|---:|---:|---:|---:|
| LF2LFY4 | 61 | 0.0 | 18.387 |
| LLY6BXN | 64 | 0.0 | 12.06 |
| LFRBDBI | 66 | 0.0 | 10.645 |
| LV5AXUD | 83 | 0.0 | 9.231 |
| LYMZ238 | 76 | 0.0 | 7.593 |
| LVSJWK7 | 86 | 0.0 | 6.625 |
| L5WN4R3 | 81 | 0.0 | 6.328 |
| L60JUKI | 61 | 0.0 | 6.253 |
| LS6BK62 | 87 | 0.0 | 5.583 |
| LJIDGYJ | 78 | 0.0 | 5.36 |


---

## Transaction Activity (summary)



### LVSJWK7

Transaction Activity:
- First transaction: Sep 05, 2025
- Most recent transaction: Sep 05, 2025
- Last transaction details: Sep 05, 2025 17:39 — collect_points (value=8.90, points=9)
- Transaction pattern: no recent transactions
- Average transaction value: ~8.90 BD
- Current status: At Risk — needs re-engagement


### L60JUKI

Transaction Activity:
- First transaction: Sep 30, 2025
- Most recent transaction: Sep 30, 2025
- Last transaction details: Sep 30, 2025 10:03 — collect_points (value=8.40, points=8)
- Transaction pattern: no recent transactions
- Average transaction value: ~8.40 BD
- Current status: At Risk — needs re-engagement


### LJIDGYJ

Transaction Activity:
- First transaction: Sep 13, 2025
- Most recent transaction: Sep 13, 2025
- Last transaction details: Sep 13, 2025 09:43 — collect_points (value=7.20, points=7)
- Transaction pattern: no recent transactions
- Average transaction value: ~7.20 BD
- Current status: At Risk — needs re-engagement


### LL3HD26

Transaction Activity:
- First transaction: Apr 10, 2025
- Most recent transaction: Dec 14, 2025
- Last transaction details: Dec 14, 2025 12:23 — collect_points (value=2.70, points=3)
- Transaction pattern: typically 3–4 transactions per week
- Average transaction value: ~2.17 BD
- Current status: Active and transacting regularly


### LWAWFXI

Transaction Activity:
- First transaction: Sep 08, 2025
- Most recent transaction: Dec 15, 2025
- Last transaction details: Dec 15, 2025 18:58 — collect_points (value=3.80, points=4)
- Transaction pattern: typically 1–2 transactions per week
- Average transaction value: ~3.81 BD
- Current status: Active and transacting regularly


### LCLEMWX

Transaction Activity:
- First transaction: Apr 10, 2025
- Most recent transaction: Nov 13, 2025
- Last transaction details: Nov 13, 2025 16:52 — redeem_points (value=-7.05, points=58)
- Transaction pattern: typically 1–2 transactions per week
- Average transaction value: ~5.49 BD
- Current status: Active and transacting regularly


### LTGMVTK

Transaction Activity:
- First transaction: Apr 10, 2025
- Most recent transaction: Dec 25, 2025
- Last transaction details: Dec 25, 2025 15:36 — collect_points (value=4.10, points=4)
- Transaction pattern: typically 5+ transactions per week
- Average transaction value: ~2.48 BD
- Current status: Active and transacting regularly


### L0LX4MO

Transaction Activity:
- First transaction: Apr 11, 2025
- Most recent transaction: Dec 14, 2025
- Last transaction details: Dec 14, 2025 19:52 — redeem_points (value=-1.70, points=11)
- Transaction pattern: typically 3–4 transactions per week
- Average transaction value: ~3.16 BD
- Current status: Active and transacting regularly


### L2UXZE7

Transaction Activity:
- First transaction: Apr 17, 2025
- Most recent transaction: Dec 24, 2025
- Last transaction details: Dec 24, 2025 12:18 — redeem_points (value=-1.20, points=8)
- Transaction pattern: typically 1–2 transactions per week
- Average transaction value: ~2.85 BD
- Current status: Active and transacting regularly


### LSMG0OF

Transaction Activity:
- First transaction: Apr 19, 2025
- Most recent transaction: Dec 24, 2025
- Last transaction details: Dec 24, 2025 20:03 — collect_points (value=4.35, points=4)
- Transaction pattern: typically 1–2 transactions per week
- Average transaction value: ~5.25 BD
- Current status: Active and transacting regularly


### LS6BK62

Transaction Activity:
- First transaction: Apr 28, 2025
- Most recent transaction: Sep 04, 2025
- Last transaction details: Sep 04, 2025 19:44 — collect_points (value=7.50, points=8)
- Transaction pattern: no recent transactions
- Average transaction value: ~7.50 BD
- Current status: At Risk — needs re-engagement


### LDWPHNO

Transaction Activity:
- First transaction: May 22, 2025
- Most recent transaction: Dec 20, 2025
- Last transaction details: Dec 20, 2025 20:08 — collect_points (value=2.40, points=2)
- Transaction pattern: typically 1–2 transactions per week
- Average transaction value: ~2.69 BD
- Current status: Active and transacting regularly


### L5WN4R3

Transaction Activity:
- First transaction: Jun 24, 2025
- Most recent transaction: Sep 10, 2025
- Last transaction details: Sep 10, 2025 19:49 — collect_points (value=3.40, points=3)
- Transaction pattern: no recent transactions
- Average transaction value: ~3.40 BD
- Current status: At Risk — needs re-engagement


### LLY6BXN

Transaction Activity:
- First transaction: Jun 25, 2025
- Most recent transaction: Sep 27, 2025
- Last transaction details: Sep 27, 2025 12:13 — collect_points (value=7.70, points=8)
- Transaction pattern: no recent transactions
- Average transaction value: ~8.10 BD
- Current status: At Risk — needs re-engagement


### LV5UT4X

Transaction Activity:
- First transaction: Jul 04, 2025
- Most recent transaction: Dec 21, 2025
- Last transaction details: Dec 21, 2025 14:47 — collect_points (value=1.70, points=2)
- Transaction pattern: typically 3–4 transactions per week
- Average transaction value: ~2.21 BD
- Current status: Active and transacting regularly


### L4VAPE9

Transaction Activity:
- First transaction: Jul 14, 2025
- Most recent transaction: Dec 15, 2025
- Last transaction details: Dec 15, 2025 12:35 — collect_points (value=3.80, points=4)
- Transaction pattern: typically 1–2 transactions per week
- Average transaction value: ~3.60 BD
- Current status: Active with low recent transaction activity


### LV5AXUD

Transaction Activity:
- First transaction: Jul 28, 2025
- Most recent transaction: Sep 08, 2025
- Last transaction details: Sep 08, 2025 18:47 — collect_points (value=1.50, points=2)
- Transaction pattern: no recent transactions
- Average transaction value: ~3.65 BD
- Current status: At Risk — needs re-engagement


### LYMZ238

Transaction Activity:
- First transaction: Aug 20, 2025
- Most recent transaction: Sep 15, 2025
- Last transaction details: Sep 15, 2025 11:35 — collect_points (value=4.40, points=4)
- Transaction pattern: no recent transactions
- Average transaction value: ~3.30 BD
- Current status: At Risk — needs re-engagement


### LFRBDBI

Transaction Activity:
- First transaction: Aug 21, 2025
- Most recent transaction: Sep 25, 2025
- Last transaction details: Sep 25, 2025 12:02 — collect_points (value=4.10, points=4)
- Transaction pattern: no recent transactions
- Average transaction value: ~4.10 BD
- Current status: At Risk — needs re-engagement


### LF2LFY4

Transaction Activity:
- First transaction: Sep 25, 2025
- Most recent transaction: Sep 30, 2025
- Last transaction details: Sep 30, 2025 18:07 — redeem_points (value=-2.00, points=20)
- Transaction pattern: no recent transactions
- Average transaction value: ~5.34 BD
- Current status: At Risk — needs re-engagement




---

## Staff Activity Breakdown


This section shows staff performance and transaction activity for 2025-11.


- **Dax**: collected 59 user points, redeemed 10 - Total collected transactions amount 205.22 BD, Total redeemed transaction amount 16.30 BD

- **Fatuma**: collected 35 user points, redeemed 5 - Total collected transactions amount 114.45 BD, Total redeemed transaction amount 11.80 BD

- **Rona**: collected 46 user points, redeemed 13 - Total collected transactions amount 154.45 BD, Total redeemed transaction amount 25.65 BD

- **Sara**: collected 40 user points, redeemed 7 - Total collected transactions amount 120.15 BD, Total redeemed transaction amount 12.20 BD


### Summary Totals
- **Total collection transactions for 2025-11**: 180
- **Total redemption transactions for 2025-11**: 35


---

## Retention Recommendations Using Loyale Solutions (actionable)

### [HIGH] At-Risk Customers
**Who:** 33 customers (61–90 days since last qualifying activity)  
**What to do:**
- Flash Deals: time-limited offers (24–48h) to create urgency
- Push Notifications: “We miss you — an exclusive offer is waiting”
- Promotional Codes: single-use comeback codes (e.g., `COMEBACK15`)
- Points Boost / Credit Boost: double points on next visit
- Personalized Vouchers / Offers: based on prior purchase behavior

### [HIGH] Churned Customers
**Who:** 171 customers (> 90 days)  
**What to do:**
- “Welcome Back” campaign with strong incentive via Offers/Vouchers
- Video Offers to reintroduce the program and drive a first return visit
- Survey / feedback prompt to learn churn drivers

### [LOW] Active Customers
**Who:** 88 customers (≤ 60 days)  
**What to do:**
- Loyalty milestones + surprise rewards (points boosts, appreciation vouchers)
- Referrals / group mechanics where applicable
- Tier progress nudges (if program supports tiers)

---

## Feature Performance

**Note:** Redemption rates show engagement but **cannot prove ROI** without control groups. Use to identify popular features, not to calculate revenue impact.

**Metric Definitions:**
- **Redemption Rate:** (Redeemed Count) ÷ (Created Count)  
- **Influenced Value:** Total loyalty value from users who redeemed this feature (correlation, not causation)

| feature_code | enabled | enabled_method | created | redeemed | redemption_rate | active_users | influenced_value | notes |
|---|---:|---|---:|---:|---:|---:|---:|---|
| flash_deals | 0 | inferred_from_flash_deal_records | 0 | 0 | NULL | 0 | NULL | influenced_value_not_available_no_attribution_fields |
| offers | 0 | inferred_from_offer_records | 0 | 0 | NULL | 0 | NULL | influenced_value_not_available_no_attribution_fields |
| points_program | 1 | merchant.cardType | NULL | 35 | NULL | 22 | NULL | created_count_not_available_for_program_points; influenced_value_not_available_no_attribution_fields |
| stamps_program | 0 | merchant.cardType | NULL | 0 | NULL | 0 | NULL | created_count_not_available_for_program_stamps; influenced_value_not_available_no_attribution_fields |
| videos | 0 | inferred_from_promotional_video_records | 0 | 0 | NULL | 0 | NULL | influenced_value_not_available_no_attribution_fields |
| vouchers | 0 | inferred_from_voucher_records | 0 | 0 | NULL | 0 | NULL | influenced_value_not_available_no_attribution_fields |


![Feature Usage](/merchant_41999_features.png)

---

## Definitions used (runtime config)

| Key | Value |
|---|---|
| month | 2025-11 |
| timezone_mode | fixed_tz |
| fixed_timezone | Asia/Bahrain |
| universe_mode | qualifying_events_only |
| qualifying_event_types | ['collect_points', 'collect_stamp', 'purchase_voucher', 'redeem_points', 'redeem_stamp', 'redeem_offer', 'redeem_flash'] |
| at_risk_inactive_days | 60 |
| churn_inactive_days | 90 |
| value_basis | revenue_amount |
| include_redemptions_in_value | False |
| points_to_currency_rate | None |

| forecast_horizon_months | 6 |
| forecast_method | survival_adjusted |
| lookback_months_for_run_rate | 3 |
| discount_rate_annual | 0.1 |

---

## Data Quality & Reconciliation

### Data quality summary

- **events_in_scope_total**: 215 (info) — All event_types in stg_activity_event within the month window (and merchant filter if applicable).

- **qualifying_events_in_scope**: 215 (info) — Only event_type IN qualifying_event_types within the same window.

- **customers_classified**: 292 (info) 

- **dup_keys_merchant_customer_monthly_status**: 0 (pass) 

- **dup_keys_merchant_monthly_summary**: 0 (pass) 

- **dup_keys_merchant_feature_monthly_metrics**: 0 (pass) 

- **dup_keys_app_user_monthly_status**: 0 (pass) 

- **dup_keys_app_monthly_summary**: 0 (pass) 

- **recon_cltv_mart**: 528.315 (info) 

- **recon_cltv_raw**: 528.3150024414062 (info) 

- **recon_delta_mart_minus_raw**: -2.441406195430318e-06 (pass) 



### Event sources (top 3, in-scope)

- **user_collect_points**: 180

- **user_redeem_points**: 35



### Reconciliation
- **CLTV (mart)**: 528.315
- **Raw qualifying value (source-derived)**: 528.3150024414062
- **Delta (mart - raw)**: -2.441406195430318e-06
- **Expected reasons**:

  - excluded event types: non-qualifying events are not included in CLTV by definition

  - timezone: month windows computed in fixed_timezone then converted to UTC; DB timestamps treated as UTC

